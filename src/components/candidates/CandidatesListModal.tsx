...
const MOCK_JOB_CANDIDATES: Record<string, ModalCandidate[]> = {
  default: [
    {
      id: 'cand-1',
      name: 'Alex Morgan',
...
export const CandidatesListModal: React.FC<CandidatesListModalProps> = ({
  ...
  const candidatesList = useMemo(() => {
    if (!job) return [];
    return MOCK_JOB_CANDIDATES[job.id] || MOCK_JOB_CANDIDATES.default;
  }, [job]);
      ...
      <div className="candidates-modal-card" onClick={(e) => e.stopPropagation()}>
          ...
          <div className="candidates-modal-header-left">
            ...
            <div className="candidates-modal-meta-row">
              <span className="candidates-modal-meta-item highlight">
                <UsersIcon />
                <span>{candidatesList.length} Total Applicants</span>
              </span>
              <span>•</span>
        ...
        <div className="candidates-modal-filter-bar">
          ...
          <div className="text-xs font-semibold text-slate-500">
            Showing <strong className="text-slate-800">{filteredCandidates.length}</strong> of {candidatesList.length} applicants
          </div>
        </div>
        ...
        <div className="candidates-modal-body">
          {filteredCandidates.length === 0 ? (
            ...
            filteredCandidates.map((candidate) => {
              ...
              return (
                <div key={candidate.id} className="candidates-modal-row">
                  {/* Candidate Identity */}
                  ...
                  {/* Right Meta: Status Badge & AI Score Placeholder */}
                  <div className="candidates-modal-row-right">
                    ...
                    {/* AI Match Score Placeholder Area */}
                    <div
                      className="flex items-center"
                      title="AI Match Score will be calculated when running AI Screening on the pipeline board"
                    >
